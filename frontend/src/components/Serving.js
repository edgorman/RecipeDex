    ...
    if (this.props.value !== -1) {
      return (
        <form id="serving" className="input-group w-auto justify-content-center align-items-center" onSubmit={this.onSubmit}>
          <input type="button" value="-" onClick={this.decreaseValue} className="button-minus rounded-circle icon-shape icon-sm me-1" data-field="quantity"/>
          <input type="number" step="1" max="99" onBlur={this.onChange} onChange={this.onChange} value={this.props.value} name="quantity" className="quantity-field border-0 text-center col-5"/>
